...
class Install(install_misc.InstallBase):
    ...
    def run_plugin(self, plugin):
        ...
        self.next_region()
        syslog.syslog('run_plugin(%s)'%(plugin))
    ...
    def cleanup(self):
        ...
        # AMP: deable dconf purge on ubiquity package desinstallation
        misc.execute('sed', '-i', 's/confmodule/confmodulea/g', self.target + '/var/lib/dpkg/info/ubiquity.postrm')

        # AMP: remove unused packages
        ...
        # AMP: restore resolv.conf
        misc.execute('rm', self.target + '/etc/resolv.conf')
        misc.execute('mv', self.target + '/etc/resolv.conf.orig', self.target + '/etc/resolv.conf')
